...
exports.createMedicine = async (req, res) => {
  ...
  try {
    const { name, description, category, stock, batchNumber, expiryDate, manufacturer, 
            composition, additionalPrice, dosage } = req.body;
    ...
    // Populate the composition's raw materials to calculate price
    await savedMedicine.populate('composition.rawMaterial');
...
exports.calculatePrice = async (req, res) => {
  try {
    ...
    if (!composition || !Array.isArray(composition)) {
      return res.status(400).json({ message: 'Valid composition array is required' });
    }
