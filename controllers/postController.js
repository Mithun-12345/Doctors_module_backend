...
const IG_BUSINESS_ID = process.env.INSTAGRAM_ACCOUNT_ID;
...
exports.createPost = async (req, res) => {
  try {
    const { text, shareToInstagram } = req.body;
    let mediaUrl = null;
    let mediaType = 'text';
    ...
    const newPost = new Post({
      ...
      mediaUrl,
      mediaType,
      shareToInstagram: shareToInstagram === 'true' || shareToInstagram === true,
    });
    ...
    if (newPost.shareToInstagram && mediaUrl) {
      try {
        const containerData = {
          caption: text || '',
        ...
        // Step 2: Wait for video to finish processing (simplified: small delay for video)
        if (mediaType === 'video') {
          await new Promise((resolve) => setTimeout(resolve, 6000)); // optional: polling can be used for better accuracy
        }
        ...
        // Save IG Post ID to DB
        newPost.instagramPostId = publishRes.data.id;
        await newPost.save();
        console.log('✅ Posted to Instagram:', publishRes.data.id);
      } catch (instaError) {
        console.error('❌ Instagram post failed:', instaError.response?.data || instaError.message);
        // Optional: notify user or flag for retry
      }
    }
