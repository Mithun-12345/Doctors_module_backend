...
class PaymentController {
  static async calculateAmount(appointmentDate, patientType) {
    try {
      const paymentConfig = await PaymentConfig.findOne().sort({ createdAt: -1 });
      if (!paymentConfig) {
      ...
      if (isWeekend) {
        return isChronicPatient ? paymentConfig.weekendChronic : paymentConfig.weekendAcute;
      }
      return isChronicPatient ? paymentConfig.weekdayChronic : paymentConfig.weekdayAcute;
    } catch (error) {
  ...
  static async createDraftAppointment(req, res) {
    try {
      const { appointmentDate, timeSlot, consultingFor, fullName, consultingReason, symptom } = req.body;
  ...
  static async createPaymentOrder(req, res) {
    try {
      ...
      const amount = await PaymentController.calculateAmount(appointmentDate, patientType);
      console.log(amount);
      const receipt = `receipt_${Date.now()}_${patientId.toString().slice(0, 8)}`; 
